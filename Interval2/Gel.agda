...
module _ {ℓ1 ℓ2 : Level} (D : Set ℓ1) (S : Set ℓ2) where
 ...
 module main {A : {t : T} (e : E t) → Set ℓ} (R : (aa : {t : T} (e : E t) → A e) → Set)  where
   ...
   data Gel (t : T) : Set ℓ where
        gstrand : (aa : {t : T} (e : E t) → A e) (r : R aa) → Gel t -- AR
        gpoint : {e : E t} (a : A e) → Gel t -- EA
        gpath : {e : E t} (aa : {t : T} (e : E t) → A e) (r : R aa) → gpoint (aa e) ≡ gstrand aa r -- EAR
   ...
   module gip (t : T) where
     ...
     fore (gstrand aa r) = pinl (aa , r)
     fore (gpoint {e} a) = pinr (e , a)
     fore (gpath {e} aa r i) = ppath (e , aa , r) i

     back : Push (ff {t}) gg → Gel t
     back (pinl (aa , r)) = (gstrand aa r)
     back (pinr (e , a)) = (gpoint {e = e} a)
     back (ppath (e , aa , r) i) = (gpath {e = e} aa r i)
   ...
   gel : {aa : {t : T} (e : E t) → A e} → R aa → ((t : T) → Gel t)
   gel {aa} r t = gstrand {t} aa r

   module _ (Rdisc : (aa : {t : T} (e : E t) → A e) → disc (R aa))
            (Adisc : (t : T) (e : E t) → disc (A e))
            (AAdisc : disc ({t : T} (e : E t) → A e)) -- should follow from Adisc I think?
     where
     ...
     ARdisc : disc AR
     ARdisc = ΣPresDisc T AAdisc Rdisc

     EARdisc : (t : T) → disc (EAR t)
     EARdisc t = prodPresDisc T (Edisc t) ARdisc

     EAdisc : (t : T) → disc (EA t)
   ...
   --   ERdisc t = prodPresDisc T (Edisc t) Rdisc
     ...
     Commute = ▻Commute (λ _ → ARdisc) EAdisc EARdisc ff gg
